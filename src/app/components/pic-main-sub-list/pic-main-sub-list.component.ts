...
export class PicMainSubListComponent implements OnInit {
  ...
  omImgSelectedToBuy(img) {
    ...
    this.buyingSvc.listOfItemToBeDisplay.push([{id: id,des: des,price: price, url: url}]);
    for (const i of this.buyingSvc.listOfItemToBeDisplay) {
      for (const j of i) {
        console.log(j.url)
    ...
    //   this.buyingSvc.bagIsNotEmpty = true;
    //   console.log(this.listOfItemsInBag.length);
